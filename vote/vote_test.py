...
img_path = os.path.join(curr_dir, 'test_event_image.png')
test_path = os.path.join(base_dir, 'eventinmaking.json')
...
event_test = json.loads(open(test_path).read())
action_s = event_test['action_s'][vote]

select_result_prob = event_test['result_prob'][vote]
possible_results = event_test['result_s'][vote]
result_s = np.random.choice(possible_results, p=select_result_prob)

previous_event = dict(result_s=result_s,
                      action_s=action_s,
                      strings=dict())

action_slen = len(event_test['action_s'])
requirements = ["" for i in range(action_slen)]

current_event = dict(event_s=event_test['event_s'],
                     action_s=event_test['action_s'],
                     requirements=requirements,
                     strings=event_test['strings'])
